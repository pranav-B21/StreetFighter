...
//class for the medium level of the game. 
//the difference compared to the easy level is that in this game mode the opponent deals more damage, but you are allowed to use your power up.
public class Medium extends JPanel implements KeyListener {
	// fields

	// gets all bg's
	private Image bg = new ImageIcon("mediumBG.gif").getImage();
	...
	public Medium() {
		...
		cpu = new CPU(width, height,"medium");
	...
	//getters and setters
	public void medPressed(boolean medPressed) {//I pass in a boolean to the cpu class whether or not the button(if clicked medium) has been pressed
		this.medPressed = medPressed;
		cpu.medPressed(medPressed);
	}
}
